...
def source_hashes():
    paths = [Path(__file__), HERE / "pareto_online.py", HERE / "setup_model_free_reductionist.json",
    ...
    paths += [HERE / f"reductionist {algorithm}.py" for algorithm in ("Pareto UCB", "Scalarized UCB", "Scalarized KG")]
...
def load_setup():
    setup = json.loads((HERE / "setup_model_free_reductionist.json").read_text(encoding="utf-8"))
