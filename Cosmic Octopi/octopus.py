...
class Octopus:
    ...
    def step(self, 
        ...
        if upload_condition:
            model = None
            if class_id == 0:
                model = copy.deepcopy(self.cheetah_model)
            if class_id == 1:
                model = copy.deepcopy(self.ant_model)
            if class_id == 2:
                model = copy.deepcopy(self.leg_model)
            if class_id == 3:
                model = copy.deepcopy(self.humanoid_model)
            return (freshness, local_fitness, model, n_episodes, max(rewards))
        return max(rewards)
