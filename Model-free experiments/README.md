...
The command displays an ASCII progress bar in CMD: completed experiments out of
120, elapsed time, estimated time remaining, and the active run/algorithm/setting.
H and R denote holistic and reductionist; grav, sync, and hete denote the three
sweeps. The status reports the active RTS/STR formulation, sweep value, and latest
hypervolume checkpoint out of 150. It updates at each saved checkpoint (every 5
online requests); the overall bar advances once the experiment is saved in the
combined pickle. Resuming starts from the completed experiment count. The time
estimate becomes available after the first newly completed experiment and varies
with the setting. Console progress creates no log file.
