...
class StubBackend:
    ...
    def save_result(self, checkpoint, result):
    ...
    def run(self, setup, checkpoint, entrypoint, executor=None):
        ...
        if checkpoint.exists():
            ...
            self.resumed = True
        result = {"setup": setup, "complete": False,
                  "runs": {"RTS/gamma_grav=1": {"checkpoint_iterations": [5]}}}
        self.save_result(checkpoint, result)
    ...
    def test_only_one_output_for_120_experiments_and_complete_resume_is_read_only(self):
        ...
        with TemporaryDirectory() as directory, redirect_stdout(io.StringIO()), redirect_stderr(console):
            ...
            with patch.object(suite, "HERE", folder):
                result = suite.run_suite(self.config, backends, show_progress=True)
                ...
                self.assertIn("Run 10/10 KG", console.getvalue())
                self.assertIn("RTS/1: 5/150", console.getvalue())
        ...
        with TemporaryDirectory() as directory, redirect_stdout(io.StringIO()), redirect_stderr(console):
            ...
            with patch.object(suite, "HERE", folder):
                with self.assertRaises(KeyboardInterrupt):
                    suite.run_suite(self.config, backends, show_progress=True)
