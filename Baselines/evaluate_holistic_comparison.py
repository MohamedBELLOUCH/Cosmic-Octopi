"""Held-out evaluation of the five completed holistic Pareto sets.

The original setup and search results are read-only. A separate resumable cache
stores every simulated cost. The baseline uses standard 2D hypervolume of mean
costs, as agreed after optimization. RTS has the same exact mean-cost area;
STR uses the existing expected-length definition with the configured directions.
"""
...
from holistic_model_based_runner import save_checkpoint, validate_setup


HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
CACHE_NAME = "holistic hypervolume comparison.pkl"
GROUPS = (
    ("MORBO RTS", "MORBO", "RTS"),
    ("qNParEGO RTS", "qNParEGO", "RTS"),
    ("MORBO STR", "MORBO", "STR"),
    ("qNParEGO STR", "qNParEGO", "STR"),
    ("Constant margin", "Constant margin", "standard"),
)
...
def prepare_inputs():
    setup_path = HERE / "setup_model_free_holistic.json"
    setup = json.loads(setup_path.read_text(encoding="utf-8"))
    validate_setup(setup)
    evaluation = deepcopy(setup["held_out_evaluation"])
    # Preserve the historical setup/search snapshots. Their null choice was
    # resolved by the user after these optimizations: standard baseline HV.
    if evaluation["baseline_hypervolume_formulation"] not in (None, "standard"):
        raise ValueError("This comparison uses the agreed standard baseline hypervolume")
    evaluation["baseline_hypervolume_formulation"] = "standard"
    evaluation["note"] = "Standard baseline hypervolume of mean held-out costs, selected by the user after optimization."
    ...
    for label, algorithm, formulation in GROUPS:
        ...
        if formulation == "standard":
            if record["kind"] != "holistic_constant_fitness_margin_pareto_set":
            ...
            x = np.repeat(unit[:, None], setup["holistic"]["horizon"], axis=1)
        ...
        if x.ndim != 2 or not len(x) or x.shape[1] != setup["holistic"]["horizon"] or not np.isfinite(x).all() or np.any((x < 0) | (x > 1)):
    ...
    # Exclude even 32-bit schedule-seed collisions with every training run.
    rng = np.random.default_rng(evaluation["seed"])
    held_out_seeds = []
    while len(held_out_seeds) < evaluation["n_realizations_per_solution"]:
        seed = int(rng.integers(0, 2**32))
        if seed not in training_seeds and seed not in held_out_seeds:
            held_out_seeds.append(seed)
    sources = [Path(__file__), HERE / "holistic_model_based_runner.py",
               ROOT / "Pareto fronts" / "script.py", ROOT / "Cosmic Octopi" / "utils.py",
               ROOT / "Cosmic Octopi" / "simulated_reward_experiment.py",
               ROOT / "Cosmic Octopi" / "front_utils.py", ROOT / "MORBO" / "script_holistic_gravity.py"]
    metadata = {"schema_version": 1, "kind": "holistic_held_out_hypervolume_comparison",
...
def worker_evaluate(indices):
    ...
    costs = module.evaluate_sequence(setup, matrices, sizes, sequences[sequence_index], "holistic", seeds[repetition])
...
def comparison_statistics(result):
    ...
    for label, _, _ in GROUPS:
        ...
        exact = standard_hypervolume(mean_costs, reference)
        polar_rts = core.estimated_hypervolume(costs, "RTS", directions, reference)
        polar_str = core.estimated_hypervolume(costs, "STR", directions, reference)
        formulation = group["formulation"]
        summary.append({"label": label, "algorithm": group["algorithm"], "formulation": formulation,
                        "n_solutions": group["n_solutions"], "n_realizations": costs.shape[1],
                        "hypervolume": polar_str if formulation == "STR" else exact,
                        "metric": f"STR expected-length ({len(directions)} directions)" if formulation == "STR" else "Exact 2D hypervolume of mean costs",
                        "standard_hypervolume_of_means": exact, "rts_quadrature_hypervolume": polar_rts,
                        "str_hypervolume": polar_str,
