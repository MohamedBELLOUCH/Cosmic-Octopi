"""Online RTS/STR scalarized KG for the holistic gravity experiment.
...
from experiment_runner import (
    ...
    get_model_size_in_kb, save_result, unit_directions,
...
def source_fingerprints():
    ...
    for name in ("RTS_Scalarized_KG.py", "STR_Scalarized_KG.py", "script_holistic_gravity.py"):
...
def validate_setup(setup):
    if (setup["simulated_rewards"] is not True or setup["n_independent_runs"] != 1
            or setup["approach"] != "holistic" or setup["experiment"] != "gravity"):
        raise ValueError("This pilot supports one simulated-reward holistic gravity run")
    b, e = setup["bandit"], setup["evaluation"]
    for name, value in {"horizon": setup["holistic"]["horizon"],
    ...
    if (b["arm_interval"] != [0, 1] or setup["holistic"]["margin_parameterization"] != "normalized_sequence"
            or np.any(np.asarray(b["cost_reference"]) >= 0)):
        raise ValueError("Use normalized holistic arms and a strictly negative cost reference")
    ...
    if (levels.ndim != 1 or not len(levels) or not np.all(np.isfinite(levels) & (levels > 0))
            ...
            or setup["sweep"]["parameter"] != "gamma_grav" or setup["sweep"]["values"] != levels.tolist()):
        raise ValueError("Expected distinct positive gravity variances and a matching sweep")
...
def train_online(setup, formulation, level, xi, model_sizes, arms, weights):
    ...
    condition["holistic"]["horizon"] = setup["bandit"]["n_online_iterations"]
    ...
    totals = run_online(condition, xi, model_sizes, environment_seed, select_margin, observe, "holistic")
    for name in history:
    ...
    return {**history, "formulation": formulation, "sweep_parameter": "gamma_grav", "sweep_value": level,
...
def run(setup, output_path, executor=None):
    ...
    if output_path.exists():
        ...
        if (result.get("schema_version") != 1 or result.get("algorithm") != "Scalarized KG"
        ...
        result = {"schema_version": 1, "algorithm": "Scalarized KG", "approach": "holistic",
                  "experiment": "gravity", "setup": setup, "source_sha256": hashes,
