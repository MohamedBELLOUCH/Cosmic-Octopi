"""Generate the Ant gravity-variance experiment's simulated-reward results."""
...
def main():
    ...
    scenarios = [
        run_scenario(
            setup, xi_matrix, setup["n_octopi"], gravity_variance,
            rng.integers(0, 2**63, size=setup["n_repetitions"]),
        )
        for gravity_variance in setup["gamma_grav_values"]
    ]
    result = {"setup": setup, "xi_matrix": xi_matrix, "scenarios": scenarios}
    output_path = EXPERIMENT_DIR / setup["output_file"]
    with output_path.open("wb") as file:
        pickle.dump(result, file, protocol=pickle.HIGHEST_PROTOCOL)
    print(f"Saved {len(scenarios)} gravity-variance scenarios to {output_path}")
