...
def load_xi_matrices(setup, experiment_dir):
    """Accept an embedded calibration for self-contained batch configurations."""
    if "Xi_matrices" in setup:
        matrices = {name: np.asarray(value, dtype=float) for name, value in setup["Xi_matrices"].items()}
    else:
        matrices = generate_Xi_matrices(1, Path(experiment_dir) / setup["xi_matrix_file"])[0]
    if set(matrices) != set(CLASS_NAMES):
        raise ValueError("Xi matrices must contain exactly the four simulator classes")
    if any(value.shape != (4, 4) or not np.isfinite(value).all() for value in matrices.values()):
...
def validate_setup(setup):
    ...
    if (bandit["arm_interval"] != expected_interval or bandit["arm_discretization"] != "linspace_inclusive"
            ...
            or bandit["observation_mode"] != "cost"
            or np.asarray(bandit["cost_reference"]).shape != (2,)
            or not np.all(np.isfinite(bandit["cost_reference"]))
            or np.any(np.asarray(bandit["cost_reference"]) >= 0)):
        raise ValueError("Unsupported arm parameterization, initialization, direction rule, or reference")
    if bandit["observation_normalization"] != "increment_divided_by_maximum_possible_increment":
        raise ValueError("Unsupported online cost normalization")
...
def make_bandit(setup, formulation, weights, seed):
    ...
    kwargs = dict(K=settings["n_arms"], D=2, weights_list=weights,
                  z=np.array(settings["cost_reference"]), observation_mode="cost", seed=seed,
...
def train_online(setup, formulation, level, xi, model_sizes, arms, weights):
    ...
    def observe(class_index, k, unit_margin, costs):
        # Match KG: bounded costs are minimized via the paper's Eq. 26.
...
def run(setup, output_path, entrypoint, executor=None):
    ...
    if output_path.exists():
        ...
        if (result.get("schema_version") != 3 or result.get("setup") != setup
        ...
        result = {"schema_version": 3, "algorithm": "Scalarized UCB", "approach": setup["approach"],
    ...
    interval = setup["evaluation"]["checkpoint_every"]
    checkpoints = sorted(set(range(interval, horizon + 1, interval)) | {horizon})
