...
Scores are maximized. Set observation_mode="cost" for the paper's negative
max-length cost scalarization; the default "reward" preserves the original toy.
The experiment setups now select cost mode. ``rho`` is retained for notebook compatibility but is
...
class _ScalarizedUCBBase:
    def __init__(self, K, D, weights_list, z=None, rho=0.01, *, seed=None,
        ...
        if observation_mode not in ("reward", "cost"):
            raise ValueError("observation_mode must be reward or cost")
    ...
    def scalar_score(self, observation, j):
        """A maximized score: Eq. 26 cost minimization or legacy toy reward mode."""
        if self.observation_mode == "cost":
            return -float(np.max(np.maximum(np.asarray(observation) - self.z, 0.0)
                                 / self.weights_list[j]))
        return length_scalarization(observation, self.weights_list[j], self.z)
...
class RTS_ScalarizedMultiObjectiveUCB(_ScalarizedUCBBase):
    def __init__(self, K, D, weights_list, z=None, rho=0.01, *, seed=None,
                 ...
                 mean_scope="global", observation_mode="reward"):
    ...
    def update(self, arm, j, reward_vec):
        ...
        if self.mean_scope == "global":
            for k, target in enumerate(self.scalar_ucbs):
                target["mean_scalar"][arm] = self.scalar_score(self.mean_vec[arm], k)
        else:
            sub["mean_scalar"][arm] = self.scalar_score(sub["mean_vec"][arm], j)
