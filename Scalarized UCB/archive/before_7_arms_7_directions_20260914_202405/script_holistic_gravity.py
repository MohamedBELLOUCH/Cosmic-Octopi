"""Online RTS/STR scalarized UCB for holistic gravity heterogeneity.

Run: python "Scalarized UCB/script_holistic_gravity.py"
The JSON defines the online horizon, finite arms, UCB settings, and a separate
held-out evaluation budget. Checkpoints contain raw trajectories, recommended
subsets, sampled evaluation sequences and costs, and both hypervolume estimates.
Rerunning resumes matching evaluation checkpoints; no Bayesian optimizer runs.
"""
...
def source_fingerprints():
    ...
    for name in ("script_holistic_gravity.py", "RTS_Scalarized_UCB.py",
                 "STR_Scalarized_UCB.py", "holistic_environment.py", "bandits.ipynb"):
...
def validate_setup(setup):
    ...
    for name, value in {
        "horizon": setup["holistic"]["horizon"], "n_arms": bandit["n_arms"],
    ...
    if bandit["n_arms"] * bandit["n_scalarization_directions"] >= setup["holistic"]["horizon"]:
        raise ValueError("Leave iterations after initialization for adaptive UCB choices")
    if (bandit["arm_interval"] != [0, 1] or bandit["arm_discretization"] != "linspace_inclusive"
            or setup["holistic"]["margin_parameterization"] != "normalized_sequence"
    ...
    levels = np.asarray(setup["gravity_variances"], dtype=float)
    if not len(levels) or not np.all(np.isfinite(levels) & (levels > 0)) or len(set(levels)) != len(levels):
        raise ValueError("Gravity variances must be distinct, finite, and positive")
...
def train_online(setup, formulation, level, xi, model_sizes, arms, weights):
    ...
    condition["system_dynamics_parameters"]["gamma_grav"] = level
    ...
    bounds = np.array([setup["system_dynamics_parameters"]["n_octopi"] * max(model_sizes), 1.0])
    ...
    totals = run_holistic_online(condition, xi, model_sizes, environment_seed, select_margin, observe)
    for name in history:
        if name != "recommended_arms":
            history[name] = np.asarray(history[name])
    if len(history["arm_history"]) != setup["holistic"]["horizon"]:
        raise ValueError("Online simulator did not provide the requested number of global iterations")
    return {
        **history, "formulation": formulation, "gravity_variance": level,
...
def evaluation_design(setup):
    ...
    engine = torch.quasirandom.SobolEngine(setup["holistic"]["horizon"], scramble=True,
                                         seed=settings["seed"])
...
def evaluate_subset(setup, level, subset, arms, draws, seeds, xi, model_sizes, executor=None):
    ...
    if setup["evaluation"]["include_constant_sequences"]:
        constants = np.repeat(subset[:, None], setup["holistic"]["horizon"], axis=1)
    ...
    for i, sequence in enumerate(sequences):
        for j, seed in enumerate(seeds):
            args = (condition, xi, model_sizes, sequence, "holistic", int(seed))
            if executor is None:
    ...
    return {
        "gravity_variance": level, "recommended_arms": subset.copy(),
        "arm_sequences": indices, "X_unit": sequences, "scenario_seeds": seeds.copy(),
...
def run(setup, output_path, executor=None):
    import torch
    ...
    if output_path.exists():
        ...
        if (result.get("schema_version") != 1 or result.get("setup") != setup
        ...
        result = {"schema_version": 1, "algorithm": "Scalarized UCB", "approach": "holistic",
                  "experiment": "gravity", "setup": setup, "source_sha256": fingerprints,
    ...
    for level in setup["gravity_variances"]:
        for formulation in setup["formulations"]:
            key = f"{formulation}/gamma_grav={level:g}"
            ...
            for iteration in checkpoints[len(trajectory["checkpoint_iterations"]):]:
                ...
                cache_key = f"gamma_grav={level:g}/arms=" + ",".join(map(str, subset))
...
def main():
    ...
    parser.add_argument("--setup", type=Path, default=EXPERIMENT_DIR / "setup_holistic_gravity.json")
    ...
    with ProcessPoolExecutor(max_workers=setup["evaluation"]["workers"],
        ...
        run(setup, EXPERIMENT_DIR / setup["output_file"], executor)
