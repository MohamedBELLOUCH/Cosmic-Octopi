...
sys.path.append(str(ROOT / "Model-free baseline"))
from model_free_quantized import load_quantized_setup, simulator_inputs, payload_measurements, extra_source_paths
...
def source_hashes():
    paths = [Path(__file__), ROOT / "Model-free baseline" / "pareto_online.py", HERE / "setup_model_free_reductionist_quantized.json",
    ...
    paths += [HERE / f"reductionist {algorithm} quantized.py" for algorithm in ("Pareto UCB", "Scalarized UCB", "Scalarized KG")]
    paths += extra_source_paths(HERE / "setup_model_free_reductionist_quantized.json")
...
def load_setup():
    setup = load_quantized_setup(HERE / "setup_model_free_reductionist_quantized.json")
    if setup["simulated_rewards"] is not True or setup["n_independent_runs"] != 1 or setup["approach"] != "reductionist":
        raise ValueError("Expected one reductionist simulated-reward run")
    ...
    if setup["formulations"] != ["RTS", "STR"] or setup["reductionist"]["margin_parameterization"] != "physical_sequence":
        raise ValueError("Expected RTS/STR and physical reductionist simulation sequences")
    ...
    for v in (setup["reductionist"]["horizon"], e["workers"], e["n_random_sequences"], e["n_realizations_per_sequence"]):
...
def unit_to_physical(setup, unit):
    unit = np.asarray(unit, dtype=float)
    if not np.isfinite(unit).all() or np.any((unit < 0) | (unit > 1)):
        raise ValueError("Expected normalized margins in [0, 1]")
...
def train_pareto(setup, xi, sizes, arms):
    ...
    condition["reductionist"]["horizon"] = setup["n_online_iterations"]
    ...
    bounds = np.array([setup["system_dynamics_parameters"]["n_octopi"] * sizes[CLASS_NAMES.index(setup["reductionist"]["class_name"])], 1.0])
    ...
    totals = run_online(condition, xi, sizes, environment_seed, select, observe, "reductionist")
...
def train(algorithm, only_formulation=None):
    ...
    physical_arms = unit_to_physical(setup, arms)
    ...
    for formulation in tqdm(formulations, desc=algorithm, unit="training run", ascii=True):
        ...
        if algorithm == "Pareto UCB":
            training = train_pareto(setup, xi, sizes, physical_arms)
        else:
            ...
            training = runner.train_online(scalarized_setup(setup, algorithm), formulation,
                       setup["system_dynamics_parameters"]["gamma_grav"], xi, sizes, physical_arms, weights)
        ...
        result = {"schema_version": 1, "kind": "reductionist_model_free_recommended_arm_set",
                  ...
                  "payload_measurements": payload_measurements(setup),
                  "compression": "int8",
                  "arms_unit": arms, "arms_physical": physical_arms, "recommended_arm_indices": subset,
                  "recommended_unit_margins": arms[subset],
                  "recommended_physical_margins": physical_arms[subset],
                  "recommended_physical_margins_by_class": {
                      name: low + (high-low)*arms[subset]
                      for name,(low,high) in setup["fitness_margin_bounds"].items() if name == setup["reductionist"]["class_name"]},
