"""Quantized holistic comparison of MORBO, qNParEGO and the threshold baseline.
...
from holistic_model_based_runner_quantized import save_checkpoint, validate_setup


HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
CACHE_NAME = "holistic model-based hypervolume comparison quantized.pkl"
GROUPS = (
    ("MORBO RTS", "MORBO", "RTS"),
...
def prepare_inputs():
    setup_path = HERE / "setup_model_based_holistic_quantized.json"
    ...
    for label, algorithm, formulation in GROUPS:
        ...
        if formulation == "standard":
            ...
            x = sample_baseline_sequences(margins, (low, high), setup, setup["holistic"]["horizon"])
    ...
    # Match the actual saved standard seeds exactly; reject training collisions.
    ...
    sources = [Path(__file__), HERE / "holistic_model_based_runner_quantized.py",
