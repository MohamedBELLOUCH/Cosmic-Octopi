...
def source_hashes():
    paths = [Path(__file__), ROOT / "Model-free baseline" / "pareto_online.py", HERE / "setup_model_free_holistic_quantized.json",
    ...
    paths += [HERE / f"holistic {algorithm} quantized.py" for algorithm in ("Pareto UCB", "Scalarized UCB", "Scalarized KG")]
    paths += extra_source_paths(HERE / "setup_model_free_holistic_quantized.json")
...
def load_setup():
    setup = load_quantized_setup(HERE / "setup_model_free_holistic_quantized.json")
