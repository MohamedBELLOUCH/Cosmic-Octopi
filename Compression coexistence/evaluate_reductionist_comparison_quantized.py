...
from model_free_quantized import compare_standard, payload_measurements
from reductionist_runner_quantized import HERE, ROOT, load_setup, save, file_hash, check_sources, unit_to_physical
...
def prepare():
    ...
    draws = np.random.default_rng(cfg["sampling_seed"]).random((cfg["n_random_sequences"], setup["reductionist"]["horizon"]))
    ...
    metadata = {"schema_version": 1, "kind": "reductionist_model_free_baseline_comparison",
...
def worker_evaluate(job):
    ...
    cost = simulator.evaluate_sequence(setup, xi, sizes, unit_to_physical(setup, sequences[i]), "reductionist", int(seeds[j]))
...
def evaluate(workers=None):
    ...
    result["payload_measurements"] = payload_measurements(setup)
    result["elapsed_seconds"] = previous + time.perf_counter()-start
    save(path, result)
    for row in result["summary"]:
        print(f"{row['algorithm']} {row['formulation']}: {row['hypervolume']:.6f}")
    for row in result["standard_comparison"]:
        print(f"{row['algorithm']} {row['formulation']}: standard={row['standard_hypervolume']:.6f}, quantized={row['quantized_hypervolume']:.6f}, difference={row['difference']:+.6f}")
    print(f"Saved {path}")
    return result
