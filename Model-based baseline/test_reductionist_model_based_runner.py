...
import reductionist_model_based_runner as runner


class ReductionistComparisonTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.setup = json.loads((runner.EXPERIMENT_DIR / "setup_model_free_reductionist.json").read_text())
    ...
    def test_real_run_loops_resume_and_do_not_touch_existing_outputs(self):
        ...
        for algorithm, backend in (("MORBO", self.morbo), ("qNParEGO", self.qnparego)):
            evaluator_name = "evaluate_candidate" if algorithm == "MORBO" else "evaluate_reductionist_candidate"
            fingerprints = runner.source_fingerprints(algorithm, runner.EXPERIMENT_DIR / f"reductionist {algorithm}.py")
            for formulation in ("RTS", "STR"):
                with self.subTest(algorithm=algorithm, formulation=formulation), tempfile.TemporaryDirectory(prefix="cosmic_baseline_test_") as directory:
                    ...
                    def fake_evaluate(x, scenario_rng, config, matrices, sizes):
                        ...
                        self.assertEqual(config["reductionist"], setup["reductionist"])
    ...
    def test_reductionist_optimizer_and_worker_use_physical_margins(self):
        import evaluate_reductionist_comparison as evaluation
        from types import SimpleNamespace
        setup = self.small_setup()
        seen = []
        def observe(config, matrices, sizes, sequence, mode, seed):
            seen.append((np.asarray(sequence).copy(), mode))
            return np.array([10., 2.])
        x = np.array([0., 0.5, 1.])
        with patch.object(self.morbo, "evaluate_sequence", side_effect=observe):
            self.morbo.evaluate_candidate(x, np.random.default_rng(1), setup, {}, np.ones(4))
        evaluation._worker = (SimpleNamespace(evaluate_sequence=observe), setup, {}, np.ones(4), x[None, :], [99])
        evaluation.worker_evaluate((0, 0))
        self.assertEqual(len(seen), 3)
        for physical, mode in seen:
            np.testing.assert_array_equal(physical, [-10., 5., 20.])
