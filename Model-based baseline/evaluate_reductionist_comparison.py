"""Held-out evaluation of the five completed reductionist Pareto sets.

The original setup and search results are read-only. A separate resumable cache
stores every simulated cost. The constant-margin baseline is scored under both
RTS (exact 2D hypervolume of mean costs) and STR (the existing expected-length
definition with the configured directions), using the same measurements.
"""
...
from reductionist_model_based_runner import save_checkpoint, validate_setup


HERE = Path(__file__).resolve().parent
ROOT = HERE.parent
CACHE_NAME = "reductionist hypervolume comparison.pkl"
GROUPS = (
    ("MORBO RTS", "MORBO", "RTS"),
...
def prepare_inputs():
    ...
    # Retain both baseline definitions in the evaluation cache.
    if evaluation["baseline_hypervolume_formulation"] not in (None, "standard", "both"):
    ...
    evaluation["note"] = "The constant-margin set is scored under RTS and STR using the same held-out cost realizations."
    ...
    for label, algorithm, formulation in GROUPS:
        ...
        if formulation == "standard":
            ...
            x = np.repeat(unit[:, None], setup["reductionist"]["horizon"], axis=1)
    ...
    sources = [Path(__file__), ROOT / "MORBO" / "script_holistic_gravity.py", HERE / "reductionist_model_based_runner.py",
...
def comparison_statistics(result):
    ...
    with redirect_stdout(io.StringIO()):
        core = importlib.import_module("script_reductionist_gravity")
