"""Shared runner for holistic and reductionist MORBO request heterogeneity experiments.

Scalarization, acquisition and trust-region updates are reused from the gravity
experiments. Only gamma_heter is swept; gamma_grav and gamma_sync stay fixed in
...
def run_condition(result, key, formulation, heterogeneity_level, setup, hp,
    ...
    run["sweep_parameter"] = "gamma_heter"
    run["sweep_value"] = heterogeneity_level
    ...
    condition_setup["system_dynamics_parameters"]["gamma_heter"] = heterogeneity_level
...
def main(approach, setup_filename, entrypoint):
    ...
    sweep = setup["sweep"]
    if (sweep["parameter"] != "gamma_heter" or not sweep["values"]
            or any(not np.isfinite(value) or value < 0 for value in sweep["values"])
            or len(set(sweep["values"])) != len(sweep["values"])):
        raise ValueError("Request heterogeneity levels must be distinct, finite, nonnegative values")
    ...
    if result is None:
        result = {
            "schema_version": 2,
            "profile": profile,
            "experiment": "request_heterogeneity",
            "approach": approach,
    ...
    for level in sweep["values"]:
        for formulation in setup["formulations"]:
            key = f"{formulation}/gamma_heter={level:g}"
            run_condition(result, key, formulation, level, setup, hp,
                          xi_matrices, model_sizes, output_path, approach)
    result["complete"] = True
    save_result(output_path, result)
    print(f"Saved {profile} {approach} request heterogeneity results to {output_path}", flush=True)
